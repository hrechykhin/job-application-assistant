...
def upgrade() -> None:
    ...
    op.execute("CREATE TYPE applicationstatus AS ENUM ('SAVED', 'APPLIED', 'INTERVIEW', 'OFFER', 'REJECTED')")
    op.create_table(
        "applications",
        sa.Column("id", sa.Integer(), nullable=False),
        sa.Column("user_id", sa.Integer(), nullable=False),
        ...
        sa.Column(
            "status",
            postgresql.ENUM("SAVED", "APPLIED", "INTERVIEW", "OFFER", "REJECTED", name="applicationstatus", create_type=False),
            nullable=False,
    ...
    op.create_table(
        ...
        sa.Column(
            "analysis_type",
            postgresql.ENUM("JOB_MATCH", "CV_TAILORING", "COVER_LETTER", name="analysistype", create_type=False),
            nullable=False,
        ),
